import styles from './Zerif.module.scss';
import Navbar from '../../components/Navbar/Navbar';
import Footer from '../../components/Footer/Footer';
import { Link } from 'react-router-dom';

export default function Zerif() {
  const vacancies = [
          ...
          {vacancies.map((vacancy) => (
            <div className={styles.VacancyListItems} key={vacancy.id}>
              <div className={styles.LogoInfo}>
                <img src="src\assets\t-tif-logo.svg" alt="vacancy" className={styles.VacancyLogo} />
                <div className={styles.VacancyInfo}>
                  <h3>{vacancy.title}</h3>
                  <p>Zərif Mühəndislər</p>
                </div>
              </div>
